...
/** Brand-specific PHEV SUV workshop profiles (Workshop tab of /technical); `brandId` narrows to one brand. */
export default async function PhevWorkshopProfiles({ brandId }: { brandId?: string }) {
  const data = await getData();
  const { profileByBrandId } = data;
  const scope = data.scope.filter((s) => !brandId || s.brandId === brandId);

  const researchedCount = scope.filter((s) => profileByBrandId.has(s.brandId)).length;

  return (
    <div>
      <h2 className="text-lg font-semibold mb-1">PHEV SUV workshop profiles (brand-specific)</h2>
      <p className="text-zinc-600 dark:text-zinc-400 mb-2 max-w-3xl">
        ...
        from the generic baseline below — nothing here falls back to the industry-generic standard; a
