...
async function getData(brandId?: string): Promise<{ models: IModel[]; brandNameById: Record<string, string> }> {
  ...
  const [models, brands] = await Promise.all([
    ModelSchema.find(brandId ? { brand_id: brandId } : {}).sort({ name: 1 }).lean(),
...
/** Known-issues tab of /technical (model-level data); `brandId` narrows to that brand's models. */
export default async function IssuesTab({ brandId }: { brandId?: string }) {
  const { models, brandNameById } = await getData(brandId);
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-1">Known issues</h2>
      ...
      {models.length === 0 ? (
        ...
        <KnownIssuesList rows={rows} unresearched={unresearched} hideBrandFilter={!!brandId} />
      )}
    </div>
  );
}
