...
// with 2+ PHEV/REEV SUV models in the catalog. Kept in sync manually; if the threshold
...
export default async function WorkshopPhevSuvPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-1">PHEV/REEV SUV Workshop Profiles</h1>
      <p className="text-zinc-600 dark:text-zinc-400 mb-2 max-w-3xl">
        Real, brand-specific after-sales infrastructure requirements for servicing PHEV/REEV SUV models — diagnostic
        interface, lift spec, PPE, technician prerequisites, and dealer audit checklist. Chinese-source-only
        research, scoped to brands with {MIN_MODEL_COUNT}+ PHEV/REEV SUV models in the catalog. Deliberately separate
            ...
            <div key={brandId} className="border border-zinc-200 dark:border-zinc-800 rounded-lg p-4">
              <div className="flex items-baseline justify-between mb-3">
                ...
                <span className="text-xs text-zinc-500 dark:text-zinc-400">
                  {count} PHEV/REEV SUV model{count === 1 ? "" : "s"}
                </span>
              </div>
