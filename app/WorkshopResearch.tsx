...
const FIELD_LABELS: Record<string, string> = {
  special_tools_list: "Special tools",
  hv_safety_requirements: "HV safety requirements",
  diagnostic_software_name: "Diagnostic software",
  technician_certification_required: "Technician certification",
  source: "Source",
};
...
export default function WorkshopResearch({ brandId, compact }: Props) {
  ...
  async function handleResearch() {
    ...
    try {
      const res = await fetch(`/api/brands/${brandId}/research-workshop`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? `Request failed with status ${res.status}`);

      const result = data.result;
      if (result.status !== "found" || !result.workshop_requirements) {
        setErrorMessage(result.errorMessage ?? "No confident workshop-requirements data found in Chinese sources for this brand.");
      ...
      setWorkshop(result.workshop_requirements);
      setMeta({ sourceCount: result.sourceUrls?.length ?? 0, hasGrounding: !!result.hasGrounding });
      setPhase("review");
    } catch (err) {
  ...
  async function handleApply() {
    ...
    setErrorMessage(null);
    try {
      const res = await fetch(`/api/brands/${brandId}/apply-workshop`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ workshop_requirements: workshop }),
      });
      const data = await res.json();
  ...
  const rows = workshop ? Object.keys(FIELD_LABELS).filter((k) => workshop[k] !== undefined && workshop[k] !== null) : [];
    ...
    <span onClick={(e) => e.stopPropagation()}>
      {compact ? (
        <button
          ...
          title="Research workshop/SAV requirements (Chinese sources only)"
        ...
        >
          {phase === "loading" && <Spinner />}
          🔧 Research workshop requirements
        </button>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={close}>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-sm">Workshop requirements research (Chinese sources only)</h3>
                  ...
                  {rows.map((k) => (
                    <div key={k} className="flex items-start gap-2 text-xs border border-zinc-200 dark:border-zinc-800 rounded p-1.5">
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="text-zinc-600 dark:text-zinc-400">{display(workshop?.[k])}</span>
                      </div>
                    </div>
                  ))}
                  {workshop?.confidence === "unconfirmed" && (
