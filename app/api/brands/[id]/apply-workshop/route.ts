...
// The only write path for workshop_requirements — see
// app/api/brands/[id]/apply-warranty/route.ts's header, same pattern.

const ALLOWED_FIELDS = new Set(["special_tools_list", "hv_safety_requirements", "diagnostic_software_name", "technician_certification_required", "source", "confidence"]);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const fields = (body?.workshop_requirements ?? {}) as Record<string, unknown>;
  ...
  const expected = { workshop_requirements: cleaned, workshop_requirements_last_researched_at: new Date() };
